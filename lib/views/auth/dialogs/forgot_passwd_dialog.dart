...
class _ForgotPasswdDialogState extends State<ForgotPasswdDialog> {
  ...
  Future<void> _submitCodeAndPasswd() async {
    ...
    try {
      ...
      if (success) {
        Fluttertoast.showToast(msg: "Password changed successfully. Please login with your new password.");
        Navigator.of(context).pop();
      } else {
        Fluttertoast.showToast(msg: "Invalid code or code expired. Please try again.");
      }
    } catch (e) {
      Fluttertoast.showToast(msg: "Unable to process request. Please try again later.");
    } finally {
  ...
  Future<void> _resendCode() async {
    ...
    try {
      await widget.authService.requestPasswdReset(emailCtrl.text.trim());
      _startTimer();
      Fluttertoast.showToast(msg: "If an account exists for this email, a new code will arrive.");
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: ConstrainedBox(
        ...
        child: stage == Stage.enterEmail ? _buildEmailEntry() : _buildCodeEntry(),
      ),
